...
sub dispatch_clone {
    ...
    if ( $r->method_number == Apache2::Const::M_GET ) {
        ...
        my $clone = $ad_zone->copy({ name => $ad_zone->name . ' 2' });

        $r->pnotes('session')->{msg} =
             sprintf( "Ad Zone '%s' was cloned", $ad_zone->name );
...
sub dispatch_assign {
    ...
    if ( $r->method_number == Apache2::Const::M_GET ) {
        ...
        foreach my $router (@routers) {
          ...
          my $assign = SL::Model::App->resultset('RouterAdZone')
            ->find_or_create({
        ...
        $r->pnotes('session')->{msg} =
             sprintf( "Ad Zone '%s' was assigned to all routers",
                      $ad_zone->name );
...
sub dispatch_remove {
    ...
    if ( $r->method_number == Apache2::Const::M_GET ) {
        ...
        foreach my $router (@routers) {
          ...
          my @razs = grep { $_->ad_zone->ad_size->grouping ==
                              $ad_zone->ad_size->grouping   }
...
sub dispatch_edit {
    ...
    if ( $r->method_number == Apache2::Const::M_GET ) {
        ...
        if ($req->param('zone_type') eq 'banner') {
            ...
            $constraints = { image_href => $self->valid_link(),
    ...
    if ($req->param('zone_type') eq 'code') {
        ...
        $args{'code'} = sprintf('<a href="%s"><img src="%s"></a>', $args{'link_href'}, $args{'image_href'});;
    }
...
sub dispatch_list {
    ...
    $_->mts(DateTime::Format::Pg->parse_datetime($_->mts)
            ->strftime("%a %b %e,%l:%m %p")) for @ad_zones;
