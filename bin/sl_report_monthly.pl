...
my $yesterday = $now->clone->subtract( days => 30 );
...
my $fh;
open( $fh, '> /tmp/sl/views/monthly.png' ) or die $!;
print $fh $gd->png;
close($fh);
...
open( $fh, '> /tmp/sl/clicks/monthly.png' ) or die $!;
print $fh $gd->png;
close($fh);
...
open( $fh, '> /tmp/sl/ads/monthly.png' ) or die $!;
print $fh $gd->png;
close($fh);
...
open( $fh, '> /tmp/sl/crs/monthly.png' ) or die $!;
print $fh $gd->png;
close($fh);
