...
my $yesterday = $now->clone->subtract( days => 90 );
...
my $fh;
open( $fh, '> /tmp/sl/views/quarterly.png' ) or die $!;
print $fh $gd->png;
close($fh);
...
open( $fh, '> /tmp/sl/clicks/quarterly.png' ) or die $!;
print $fh $gd->png;
close($fh);
...
open( $fh, '> /tmp/sl/ads/quarterly.png' ) or die $!;
print $fh $gd->png;
close($fh);
...
open( $fh, '> /tmp/sl/crs/quarterly.png' ) or die $!;
print $fh $gd->png;
close($fh);
