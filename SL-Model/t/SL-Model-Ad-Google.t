...
my $ok = $pkg->match_and_log( { url => $url, ip => $ip, referer => $referer,
                                 mac => $mac, user => $user, referer => $referer } );
cmp_ok( $ok, '==', $GOOGLE_AD_ID, "match_and_log() ok" );

# all this stuff below is for stealth mode

use SL::Config;
my $CONFIG = SL::Config->new;
SKIP: {
    skip 'not in google ad stealth mode', 1 unless $CONFIG->sl_google_stealth;
    require SL::Page::Cache;
    my $mog_port       = "2525";
    my $cached_url     = "http%3A%2F%2F127.0.0.1%3A$mog_port%2Ffizzbin.html";
    my $cached_ref_url =
      "http%3A%2F%2F127.0.0.1%3A$mog_port%2Ffizzbin_referer.html";

    use URI::Escape;
    my $i = 0;
    my $sub = sub {
        ( $i++ == 0 )
          ? URI::Escape::uri_unescape($cached_url)
          : URI::Escape::uri_unescape($cached_ref_url);
    };
...
"/pagead/ads?client=ca-pub-5785951125780041&dt=1186644785249&lmt=1186644784&format=728x15_0ads_al_s&output=html&correlator=1186644785249&url=$cached_url&ref=$cached_ref_url&ga_vid=1774040002.1186644785&ga_sid=1186644785&ga_hid=819217451&flash=9&u_h=1050&u_w=1680&u_ah=1024&u_aw=1680&u_cd=32&u_tz=-420&u_his=10&u_java=true&u_nplug=5&u_nmime=78";
    ...
    $i   = 0;
    $url =
'http://pagead2.googlesyndication.com/pagead/ads?client=ca-pub-5785951125780041&dt=1186644785249&lmt=1186644784&format=728x15_0ads_al_s&output=html&correlator=1186644785249&url=http%3A%2F%2Fwww.redhotpenguin.com%2F&ref=http%3A%2F%2Fwww.redhotpenguin.com%2Farchives%2F000020.html&ga_vid=1774040002.1186644785&ga_sid=1186644785&ga_hid=819217451&flash=9&u_h=1050&u_w=1680&u_ah=1024&u_aw=1680&u_cd=32&u_tz=-420&u_his=10&u_java=true&u_nplug=5&u_nmime=78';

    $new_test_uri =
"/pagead/ads?client=ca-pub-5785951125780041&dt=1186644785249&lmt=1186644784&format=728x15_0ads_al_s&output=html&correlator=1186644785249&url=$cached_url&ref=$cached_ref_url&ga_vid=1774040002.1186644785&ga_sid=1186644785&ga_hid=819217451&flash=9&u_h=1050&u_w=1680&u_ah=1024&u_aw=1680&u_cd=32&u_tz=-420&u_his=10&u_java=true&u_nplug=5&u_nmime=78";

    my $new_uri =
      $pkg->match_and_log( { url => $url, ip => $ip, referer => $referer } );
