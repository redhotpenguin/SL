...
SQL
    ...
    my $ad_zone_ids = $dbh->selectall_arrayref(<<SQL, {}, $bug->{bug_id} );
...
SQL
    ...
    foreach my $ad_zone_id (@{$ad_zone_ids}) {
      ...
      my $radzones = $dbh->selectall_arrayref(<<SQL, {}, $ad_zone_id->[0]);
SELECT router_id from router__ad_zone where ad_zone_id = ?
SQL
      foreach my $radzone ( @{$radzones}) {

        $dbh->do(sprintf("insert into router__ad_zone (router_id, ad_zone_id) values ( %s, %s ) ", $radzone->[0],
                         $bug_id));
